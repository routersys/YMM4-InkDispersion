...
public sealed class TextsTests
{
    ...
    {
        var neutral = Texts.ResourceManager.GetString(nameof(Texts.InkDispersion), CultureInfo.InvariantCulture);

        Assert.Equal("墨拡散", neutral);
        Assert.Equal(neutral, Texts.ResourceManager.GetString(nameof(Texts.InkDispersion), CultureInfo.GetCultureInfo("ja-JP")));
    }
