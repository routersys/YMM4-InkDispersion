...
static int Structure(HarnessImage image)
{
    ...
    var variants = new (string Name, Func<bool, InkDispersionPipeline.Parameters> Make)[]
    {
        ("cached", _ => parameters),
        ("water", flip => parameters with { Water = flip ? 0.70f : 0.60f }),
        ("reach", flip => parameters with { ReachPixels = flip ? 176f : 160f }),
        ("fiber", flip => parameters with { Fiber = flip ? 0.60f : 0.50f }),
        ("sizing", flip => parameters with { Sizing = flip ? 0.40f : 0.30f }),
        ("viscosity", flip => parameters with { Viscosity = flip ? 0.40f : 0.30f }),
        ("drying", flip => parameters with { Drying = flip ? 0.50f : 0.40f }),
        ("seed", flip => parameters with { Seed = flip ? 8 : 7 }),
        ("all", flip => parameters with
        {
            Water = flip ? 0.70f : 0.60f,
            ReachPixels = flip ? 176f : 160f,
            Fiber = flip ? 0.60f : 0.50f,
            Sizing = flip ? 0.40f : 0.30f,
            Viscosity = flip ? 0.40f : 0.30f,
            Drying = flip ? 0.50f : 0.40f,
            Seed = flip ? 8 : 7,
        }),
    };
