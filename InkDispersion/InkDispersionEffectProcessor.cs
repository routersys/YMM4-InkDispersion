...
internal sealed class InkDispersionEffectProcessor : VideoEffectProcessorBase
{
    ...
    private InkDispersionGpuInterop? _interop;
    ...
    {
        var interop = InkDispersionGpuInterop.TryCreate(devices);
        if (interop is null)
            return null;
        var pipeline = InkDispersionPipeline.TryCreate(interop.Device);
        if (pipeline is null)
        {
            interop.Dispose();
            return null;
