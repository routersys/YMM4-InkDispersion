...
internal sealed class InkDispersionInteropProvider : ComputeExternalDirect3D11Provider
{
    private readonly ID2D1DeviceContext6 _renderContext;

    private InkDispersionInteropProvider(
    ...
    public static InkDispersionInteropProvider? TryCreate(
        ...
        out GraphicsDevice? graphicsDevice)
    {
        ...
        try
        {
            ...
            var provider = new InkDispersionInteropProvider(device, context, renderContext, scheduler);
            renderContext = null;
            return provider;
        }
        catch
...
internal sealed partial class InkDispersionResourceSet
{
