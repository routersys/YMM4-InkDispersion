...
Texture2D InkTexture : register(t1);
SamplerState InkSampler : register(s1);
...
) : SV_TARGET
{
    ...
    float4 ink = InkTexture.SampleLevel(InkSampler, uv1.xy, 0);
    ink.rgb = min(ink.rgb, ink.a.xxx);
    ink *= amount;
    return ink + source * (1.0 - ink.a);
}
